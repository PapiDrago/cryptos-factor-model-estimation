...
\begin{frame}{Initial Approach: The Limits of Pure PCA}
    We can easily estimate common factors $\hat{F}_t$ by applying Principal Component Analysis (PCA) to the full sample of log-returns. This naturally, under our assumptions, However is not that easy:
    
    \begin{itemize}
        \item PCA successfully reduces our 5 assets into a few orthogonal components that capture the highest variance.
        \item \textbf{The Problem:} 
        \begin{itemize}
            \item \textbf{Lack of Interpretation:} These factors are just mathematical constructs; we don't know their relationship with individual asset dynamics.
            \item \textbf{No Testing Framework:} We cannot assess the "goodness of fit" or perform hypothesis testing in a static environment.
            \item \textbf{Look-ahead Bias:} Using the whole dataset to find factors prevents us from testing how the model would have performed in "real-time."
        \end{itemize}
    \end{itemize}

    \begin{block}{Transition to Stock \& Watson (2002)}
        To transform these "blind" factors into a verifiable forecasting tool, we adopt the \textbf{Stock and Watson methodology}, which allows for simulation, benchmarking, and formal evaluation.
    \end{block}
\end{frame}

\begin{frame}{Estimating the Number of Common Factors}
