...
\begin{frame}{Main Assumptions}
  For consistent estimation of the factor space and of the number of
  factors $k$ by PCA, we assume:
  \vspace{-0.25cm}
  \begin{itemize}
    \item \textbf{Asymptotics:} $\min\{N,T\}\to\infty$ jointly.
    \item \textbf{Stationarity \& moments:} $X_{i,t}$ is (weakly) stationary,
          with $\mathbb{E}\lVert F_t\rVert^{4}<\infty$ and
          $\mathbb{E}\lvert u_{i,t}\rvert^{8}<\infty$, so the sample
          covariances converge.
    \item \textbf{Factor identification:}
          $\Sigma_F=\mathbb{E}[F_tF_t']\succ 0$ (rank $k$): the factors are linearly independent.
    \item \alert{\textbf{Pervasive (strong) factors:}}
          $\dfrac{1}{N}\sum_{i=1}^{N}\phi_i\phi_i' \;\Sigma_\phi\succ 0$
          as $N\to\infty$: each factor's loadings accumulate at rate $N$.
    \item \textbf{Approximate factor structure:}
          $\lambda_{\max}(\Sigma_u)=O(1)$ as $N\to\infty$: only \emph{weak}
          cross-sectional dependence is allowed in $u_{i,t}$.
    \item \textbf{Exogeneity:} $\mathbb{E}[u_{i,t}F_t']=0$ (factors and
          idiosyncratic errors uncorrelated).
  \end{itemize}
  \end{frame}

\begin{frame}{Consequence: eigenvalue separation}
    Pervasiveness together with $\Sigma_F\succ0$ makes the top $k$
    eigenvalues \emph{diverge}, while the approximate structure keeps the
    rest bounded:
    \[
      \lambda_j(\hat\Sigma_X)\;\ge\; c_0\,N \quad (j\le k),
      \qquad
      \lambda_j(\hat\Sigma_X)\;=\;O(1) \quad (j> k).
    \]
    This widening gap is what PCA exploits to recover the factor space.
\end{frame}
...
\begin{frame}{Results: Factor Model vs. AR Benchmark}
 ...
 \vspace{-0.12cm}
    ...
    \begin{itemize}
        ...
        \end{equation*}
       It \emph{seems} the approximate-factor condition is violated here, i.e.\ $\lambda_{\max}(\Sigma_u)$ appears \textbf{unbounded}.
...
\begin{frame}{Robustness Check: Expanding the Cross-Section ($N$)}
    ...
    \begin{itemize}
        ...
        \begin{itemize}
            \item \textbf{Small Dataset ($N=5$):} We reject $H_0$ for the largest eigenvalue ($p=1$). Consequently, $\hat{k}=0$, indicating the absence of evidence about a common factor structure.
